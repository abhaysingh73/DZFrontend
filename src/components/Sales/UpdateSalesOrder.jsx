...
const UpdateSalesOrder = () => {
    ...
    const setFormData = (order) => {
        // Customer Info
        setCustomerName(order?.customer?.name || '');
        setCustomerMobile(order?.customer?.phone || '');
        setCustomerEmail(order?.customer?.email || '');
        setShippingAddress(order?.customer?.shippingAddress?.street || '');
        setBillingAddress(order?.customer?.billingAddress?.street || '');

        // Expected Dates
        setExpectedDeliveryDate(order?.shippingDate ? new Date(order.shippingDate).toISOString().split('T')[0] : '');
        ...
        const mappedItems = order.items.map(item => ({
            ...
            totalPriceWoDisc: (item.unitPrice * item.quantity) || 0,
            stock: 0, // Not present in order, optional to update later
            productId: item.productId?._id || item.productId || '',
            discount: item.discount || 0,
            image: '', // Image info not in item, optional to fetch from inventory
        ...
        setTaxAmount(order?.tax || 0);
        setShippingFee(order?.shippingCost || 0);
        setFinalTotal(order?.totalAmount || 0);
        setTotalWoDiscount((order?.items || []).reduce((acc, item) => acc + (item.unitPrice * item.quantity), 0));

        // Other fields
        setImageData(order?.images?.[0] || null);
        setNote(order?.note || '');
        ...
        setFilteredItems([]);
        setCurrentItem(0);
        setInventory([]); // Optional: you might want to fetch related inventory separately
        setTaxRate(10); // You could also calculate this from the taxAmount / pre-tax total if needed
    };
